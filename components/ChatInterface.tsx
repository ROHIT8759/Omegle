...
interface Message {
    ...
    imageTimerDuration?: number // Duration in seconds (0 for never)
    imageOpenedAt?: number // Timestamp when image was opened
}
...
export default function ChatInterface({ onBackToHome }: ChatInterfaceProps) {
    ...
    const [imageTimer, setImageTimer] = useState<number>(0) // Default: Never expires
    const [currentTime, setCurrentTime] = useState(Date.now())
    const [isBotMode, setIsBotMode] = useState(false)
    const [searchTimeout, setSearchTimeout] = useState<NodeJS.Timeout | null>(null)
    const [viewingImage, setViewingImage] = useState<string | null>(null) // For image modal
    const [botName, setBotName] = useState<string>('')
    const [botAge, setBotAge] = useState<number>(0)
    const messagesEndRef = useRef<HTMLDivElement>(null)
    const fileInputRef = useRef<HTMLInputElement>(null)

    // Bot names pool (25 girls' names)
    const botNames = [
    ...
    useEffect(() => {
        ...
        newSocket.on('matched', (data: any) => {
            ...
            setMessages([])
            console.log('Real user matched - bot mode disabled')
        })

        newSocket.on('message', (data: any) => {
            console.log('Message received:', data)
            // If receiving a real message, ensure bot mode is disabled
            if (isBotMode) {
                console.log('Received real message, disabling bot mode')
                setIsBotMode(false)
            }
